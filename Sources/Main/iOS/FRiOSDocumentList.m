...
@implementation FRiOSDocumentList
...
- (id)init
{
    ...
    if (self) {
        ...
        if ( documentsDirPath ) {
            NSURL *documentsDirURL = [NSURL fileURLWithPath:documentsDirPath];
            
            NSError *error = nil;
            NSArray<NSURL *> *docs = [[NSFileManager defaultManager] contentsOfDirectoryAtURL:documentsDirURL includingPropertiesForKeys:nil options:NSDirectoryEnumerationSkipsSubdirectoryDescendants|NSDirectoryEnumerationSkipsPackageDescendants|NSDirectoryEnumerationSkipsHiddenFiles error:&error];
            if (docs && [docs count] > 0 && !error) {
                ...
                NSLog(@"Error retrieving document list from path: %@", documentsDirURL);
            }
        }
        else {
            NSLog(@"Error retrieving path to Documents directory");
        }
    }
    return self;
}
...
{
    if (indexPath.row < (NSInteger)[[self docs] count]) {
        NSURL *url = [[self docs] objectAtIndex:indexPath.row];
        NSString *fname = [[url path] lastPathComponent];
        
        // TODO: return a checkmark cell here and configure accordingly
...
{
    // TODO: change this to tableView:didSelectRowAtIndexPath:
...
{
    // TODO: some other way of zipping the file
